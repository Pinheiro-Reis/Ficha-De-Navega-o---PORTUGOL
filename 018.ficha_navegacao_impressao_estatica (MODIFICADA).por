programa {
  ...
  funcao formataNumero(inteiro numero, inteiro tamanhoLinha){
    escreva (tx.preencher_a_esquerda('0',qtdCasaDec, ""+numero))
  }
  ...
  {
    limpa()
    cadeia linha = "+__________MENU PRINCIPAL__________+"
    inteiro tamanhoLinha = tx.numero_caracteres(linha)
    ...
    formataMensagem("4 - SAIR", tamanhoLinhaDivisoria) + " |"
	linha=""
  ...
  funcao imprimirFichaNavegacao(real qtPts, cadeia pontos[], real rumos[], real distancias[], real tempos[], real etos[])
  {
    se(navegacaoCalculada)
	{
      ...
      {
        cadeia impressao = formataMensagem(pontos[i],28) + formataNumero(rumos[i], 3) + formataNumero(distancias[i], 3) + formataNumero(tempos[i], 3) + formataNumero(etos[i],3) + "|\n"
        escreva(impressao)
        //escreva("\n\n",pontos,"\n\n")escreva(tempos)
      }
      escreva("+________________________________________________________________________________+\n")
	    ...
	    enquanto(loop == 1)
		{
		  ...
		  escolha(MenuPrincipal())
		  {
		      caso 1:
			  {
			    ...
			    {
				  //tempos[i] = m.arredondar_para_cima(distancias[i]/velocidade)
			    }
			    pare
			  }
