programa {
  ...
  funcao imprimirFichaNavegacao(real qtPts, cadeia pontos[], real rumos[], real distancias[], real tempos[], cadeia etos[])
  {
  ...
  */
	...
	funcao cadeia lerHorario(inteiro hora[])
	{
		...
		input = (tp.inteiro_para_cadeia(hora[0],10)) + ":" + (tp.inteiro_para_cadeia(hora[1],10))
		retorne input
	}
	    ...
	    enquanto(loop == 1)
		{
		  ...
		  escolha(MenuPrincipal())
		  {
			  ...
			  //{
			    ...
			    para(inteiro i = 0; i < qtPts; i++)
			    {
				  tempos[i] = (distancias[i] / velocidade) * 60
				  //tempos[i] = m.arredondar(tempos[i])
			    }
			  ...
			  //{
			  limpa()
			  se (navegacaoCalculada == verdadeiro)
			  {
			  ...
			  senao
			  {
			  	imprimeErro("OS DADOS NÃO FORAM CADASTRADOS")
			  }
				pare
			  //}
