programa {
  ...
  funcao imprimirFichaNavegacao(inteiro qtPts, cadeia pontos[], inteiro rumos[], real distancias[], real tempos[], real etos[])
  {
    se(navegacaoCalculada){
      ...
      escreva("-------------------------------------------------------------------------------- \n")  
      cadeia impressao = formataTexto(pontos[qtPts]) + "    |   " + formataNumero(rumos[qtPts], 3) +"    |       " + formataNumero(distancias[qtPts], 2) +"      |    " + formataNumero(tempos[qtPts], 2) + "     |   "+ etos[qtPts] + "   |    \n"
      escreva(impressao)
      escreva("________________________________________________________________________________ \n")
    }senao{
  ...
  /***********************************************************************************************/
	...
	funcao mudarHorario(inteiro opHora, inteiro minutos, inteiro fusoHorario, inteiro matrizHorarios)
	{
    matrizHorarios[0][repHora] = opHora + fusoHorario

    matrizHorarios[1][repHora] = minutos
    
    matrizHorarios[2][repHora] = fusoHorario
	}

  funcao imprimirTempos(inteiro qtde) {
  ...
  funcao inicio() {
    inteiro qtPts
  	...
  	leia(qtPts)
  	inteiro rumos[qtPts], horaPouso, gmt, horaDecolagem, minutoDecolagem, tempos[qtPts][3]
  	cadeia pontos[qtPts],indicativos[2]
  	real distancias[qtPts], etos[qtPts], consHora, distanciaTotal, consumoTotal
    escolha(MenuPrincipal())
    {
      caso 1: //input
      {
        ...
        continuar()
        escreva("DIGA A HORA DE DECOLAGEM: ")
        leia(horaDecolagem)
        escreva("DIGA O MINUTO DE DECOLAGEM: ")
        leia(minutoDecolagem)
        escreva("DIGA O FUSO HORARIO: ")
        leia(gmt)
        escreva(mudarHorario(horaDecolagem, minutoDecolagem, gmt, tempos))
        repHora++
      ...
      caso 2: //impressao
      {
        ...
        escreva ("DIGITE CÓDIGO DA FICHA: ")
        leia (nmrFicha)

        imprimirFichaNavegacao(nmrFicha, pontos, rumos, distancias, tempos, etos)
